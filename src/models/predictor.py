...
class AdvancedModelPredictor:
    ...
    def predict_next_candle(self, symbol: str = "BTC/USDT") -> Dict:
        ...
        try:
            ...
            # 1. Fetch latest 180+ candles from Binance
            df = self._fetch_recent_candles(symbol, limit=200)
            if df is None or len(df) < 180:
                return {"error": "Insufficient candle data", "status": "failed"}
            
            # 2. Extract features using sequence labeler
            features = self._extract_features(df.tail(180))
            if features is None:
    ...
    def _extract_features(self, candle_window: pd.DataFrame) -> Optional[np.ndarray]:
        """Extract features from 180-candle window"""
        try:
    ...
    def _get_market_context(self, recent_candles: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "price_change_10min": round(float((prices.iloc[-1] - prices.iloc[0]) / prices.iloc[0] * 100), 3)
            }
        except:
